...
function buildVisionOptions(): vscode.LanguageModelChatRequestOptions {
    ...
    const visionThinking = vscode.workspace.getConfiguration().get<boolean>("multiLLM.visionProxyThinking", false);
    if (visionThinking) {
...
async function sendToVisionModel(
    ...
    if (!models || models.length === 0) {
        throw new Error(`Vision model "${visionModelId}" not found. Check the multiLLM.visionProxyModel setting.`);
    }
